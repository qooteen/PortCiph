...
public class PortEncode {
    ...
    public void encode(String messageFile, String cryptogramFile, String keyFile) {
        ...
        String messageWithoutNonAlphabetSymbols = lowerMessage.replaceAll("[^a-я]", "");
// Превращаем строку в список букв
        ...
        List<Character> keyLetters = upperKey.chars().mapToObj(e -> (char) e).collect(Collectors.toList()); // Тоже самое делаем с ключом
    ...
    public void decode(String cryptogramFile, String messageFile, String keyFile) {
        ...
        String upperKey = key.toUpperCase().replace('Ё', 'Е'); // Переводим ключ в верхний регистр, заменяем ё на е
        List<Character> cryptogramLetters = cryptogram.chars().mapToObj(e -> (char) e).collect(Collectors.toList());  // Превращаем строку в массим букв и каждому символу е сопостовляем char от e и кладем в список
        List<Character> keyLetters = upperKey.chars().mapToObj(e -> (char) e).collect(Collectors.toList()); // Тоже самое делаем с ключом
